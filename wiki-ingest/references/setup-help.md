...
This guide helps when `wiki-config.md` or `wiki-schema.md` cannot be found. The recommended path is `/wiki-config` - it handles everything below interactively with validation, and it is the only skill that can deploy `wiki-schema.md`. For local context, see this repository's `README.md` and `wiki-config/assets/wiki-help.md`.
