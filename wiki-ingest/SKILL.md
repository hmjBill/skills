---
name: wiki-ingest
description: "将源文件处理为合成后的 wiki 页面。当用户说 /wiki-ingest、"导入这些文件"、"处理我的剪藏"、"消化这篇文章"、"我有新文章要添加"、"把这个 PDF 转成 wiki 页面"、"我的 raw/ 里有什么"时使用。将 raw/ 视为扁平队列，扫描全部文件，将知识合成到 wiki 页面，并将每个源文件移动到 ingested/ 作为原子提交。需要文件系统读写移动权限。"
metadata:
  version: "4.15"
---
