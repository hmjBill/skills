...
If you are using Obsidian, a separate TaskNotes companion skill can integrate with the wiki skills, but it is not included in this repository and is not required by the six local wiki skills. It requires the **TaskNotes Community Plugin** enabled in Obsidian. With both installed, agents can run their own Kanban boards anchored to domain homes, and domain homes gain a live task view automatically.
