...
description: 将文本内容生成 Excalidraw 图表，支持 Obsidian、标准 Excalidraw 和动画输出
---

# Excalidraw 图表生成器

将文本内容生成 Excalidraw 图表，支持多种输出格式。
...
与标准格式相同，但每个元素添加 `customData.animate` 字段控制动画顺序：
...
每个元素需要以下字段（不要添加 `frameId`、`index`、`versionNonce`、`rawText` 等额外字段——它们可能在 excalidraw.com 上导致问题。`boundElements` 必须为 `null` 而非 `[]`，`updated` 必须为 `1` 而非时间戳）：
